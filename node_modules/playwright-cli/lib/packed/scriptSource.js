...
class Recorder {
    ...
    _shouldIgnoreMouseEvent(event) {
        ...
        if (nodeName === 'SELECT')
            return true;
        if (nodeName === 'INPUT' && ['date', 'checkbox'].includes(target.type.toLowerCase()))
            return true;
        return false;
    }
    _onMouseDown(event) {
...
class Script {
    constructor(injectedScript, options) {
        ...
        if (options.enableRecorder)
            new recorder_1.Recorder(injectedScript, this._consoleAPI);
    }
}
...
function buildSelector(injectedScript, targetElement) {
    ...
    const selector = xpathSelectorEngine_1.XPathEngine.create(document.documentElement, targetElement, 'default') ||
