...
if (process.env.PWTRACE) {
    program
        ...
        traceViewer_1.showTraceViewer(resolveHome(command.resources), resolveHome(trace));
    }).on('--help', function () {
        console.log('');
        console.log('Examples:');
        console.log('');
...
async function open(options, url) {
    ...
    new scriptController_1.ScriptController(context, undefined);
...
async function codegen(options, url, target, outputFile) {
    ...
    const outputs = [new outputs_1.TerminalOutput(process.stdout, languageGenerator.highligherType())];
    if (outputFile)
        outputs.push(new outputs_1.FileOutput(outputFile));
    const output = new outputs_1.OutputMultiplexer(outputs);
    const generator = new codeGenerator_1.CodeGenerator(browserName, launchOptions, contextOptions, output, languageGenerator, options.device);
    new scriptController_1.ScriptController(context, generator);
    await openPage(context, url);
    if (process.env.PWCLI_EXIT_FOR_TEST)
        await Promise.all(context.pages().map(p => p.close()));
}
